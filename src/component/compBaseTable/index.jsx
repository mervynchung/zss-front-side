/**
 * 单实体数据表格展示组件
 * @props.data {Object} 包含样式定义的数据
 * @props.bordered {boolean} 是否有边框
 * @props.striped ｛boolean｝ 是否有间隔背景条纹
 * 参考用法: const demo = <BaseTable data={data} bordered striped/>
 */
...
const baseTable = React.createClass({
    render(){
        ...
        const tr = [];
        let td = [];
        const colGroupNum = this.props.model.colGroupNum;
        //设置colgroup样式
        for (let i = 0; i < (1 < colGroupNum < 5 ? colGroupNum : 2); i++) {
        ...
        for (let i = 0; i < this.props.model.props.length; i++) {
            ...
            if (prop.groupspan) {
                ...
                td.push(<td prop={'td-k-'+prop.id} className="prop-name">{prop.name}</td>);
                td.push(<td prop={'td-v-'+prop.id} colSpan={prop.groupspan*2-1}>{this.props.data[prop.id]}</td>);
                colCount += prop.groupspan
                //处理非跨列项目
            } else {
                td.push(<td prop={'td-k-'+prop.id} className="prop-name">{prop.name}</td>);
                td.push(<td prop={'td-v-'+prop.id}>{this.props.data[prop.id]}</td>);
                colCount += 1;
            }
            if (colCount == colGroupNum) {
                ...
                td = [];
                colCount = 0;
            } else if (i == this.props.model.length - 1) {
                tr.push(<TrWrapper key={'tr-'+tr.length+1}>{td}</TrWrapper>);
            }
        }
