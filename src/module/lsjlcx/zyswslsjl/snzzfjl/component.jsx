...
const API_URL = config.HOST + config.URI_API_PROJECT + '/swslsjl/snzzfjl1';
...
const lrb = React.createClass({
    ...
    render(){
        ...
        //定义提示内容
        let helper = [];
        helper.push(<p key="helper-0">显示审核结束的执业转非执业信息</p>);
        helper.push(<p key="helper-1">检索功能只显示前1000条记录</p>);

        return <div className="snzzfjl">
            <div className="wrap">
                {this.state.helper && <Alert message="执业人员执转非记录查询帮助"
                                             description={helper}
                                             type="info"
                                             closable
                                             onClose={this.handleHelperClose}/>}

                <Panel title="执业人员执转非记录查询" toolbar={toolbar}>
