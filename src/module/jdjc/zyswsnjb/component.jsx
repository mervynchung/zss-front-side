...
import Zyswsnjbxx from './Zyswsnjbxx'
import config from 'common/configuration'



const API_URL = config.HOST + config.URI_API_PROJECT + '/jdjc/zyswsnjb';
const ToolBar = Panel.ToolBar;
const ButtonGroup = Button.Group;


const zyswsnjb = React.createClass({
    //初始化state
    getInitialState(){
        return {
            urls: '',
    ...
    fetchData(params = {page: 1, pageSize: this.state.pagination.pageSize}){
     ...
     fetch_zyswsnjbxx(){
        req({
            ...
            this.setState({entity:resp.data,detailHide:false});
        }).fail(err=>{
            Modal.error({
                title: '数据获取错误',
                content: (
    ...
    render(){
        ...
        let helper = [];
        helper.push(<p key="helper-0">本功能主要提供年检查询</p>);
        helper.push(<p key="helper-1">查询相关人员年检记录</p>);

        return <div className="jdjc-zyswsnjb">
            <div className="wrap">
              ...
              closable >
                <Zyswsnjbxx  data={this.state.entity}/>  
                </Panel>}
            </div>
        </div>
    }
